...
export default function Reports() {
  ...
  function exportAreasToCSV(data: AreaMetrics[], filename: string) {
    const headers = ['Área', 'Usuarios Totales', 'Usuarios Activos', 'Tareas Totales', 'Completadas', '% Completado', 'Eficiencia Promedio', 'Top Performers'];
...
// Componente para métricas de áreas
function AreasMetrics({ metrics }: { metrics: AreaMetrics[] }) {
  ...
  const avgCompletion = metrics.length > 0 
    ? metrics.reduce((acc, m) => acc + m.completionRate, 0) / metrics.length 
    : 0;
  const totalTasks = metrics.reduce((acc, m) => acc + m.totalTasks, 0);  
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-indigo-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-indigo-600 font-medium">Áreas Activas</p>
              <p className="text-2xl font-bold text-indigo-900">{totalAreas}</p>
            </div>
        ...
        <div className="bg-green-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-900">{totalActiveUsers}</p>
            </div>
        ...
        <div className="bg-blue-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-blue-600 font-medium">Finalización Media</p>
              <p className="text-2xl font-bold text-blue-900">{avgCompletion.toFixed(0)}%</p>
            </div>
        ...
        <div className="bg-orange-50 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-orange-600 font-medium">Total Tareas</p>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-medium text-gray-900">Rendimiento por Área</h3>
        </div>
        
        <div className="p-6">
          <div className="space-y-6">
            {metrics.map((area) => (
              <div key={area.areaId} className="border rounded-lg p-6 hover:bg-gray-50 transition-colors">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-xl font-semibold text-gray-900">{area.areaName}</h4>
                  <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                    area.completionRate >= 80 
                      ? 'bg-green-100 text-green-800' 
                      : area.completionRate >= 60 
                      ? 'bg-yellow-100 text-yellow-800'
                      : 'bg-red-100 text-red-800'
                  }`}>
                    {area.completionRate.toFixed(1)}% Completado
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-4">
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-bold text-green-600">{area.completedTasks}/{area.totalTasks}</div>
                    <div className="text-sm text-gray-600">Tareas Completadas</div>
                  </div>
                  
                  <div className="text-center">
                    <div className="text-2xl font-bold text-purple-600">{(area.averageEfficiency * 100).toFixed(0)}%</div>
                    <div className="text-sm text-gray-600">Eficiencia Media</div>
                  </div>
                  
                  <div className="text-center">
                    <div className="text-2xl font-bold text-orange-600">{area.averageTasksPerUser.toFixed(1)}</div>
                    <div className="text-sm text-gray-600">Tareas por Usuario</div>
                  </div>
                </div>
