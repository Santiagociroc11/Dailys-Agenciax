...
const Dashboard = () => {
  ...
  async function fetchTeamMetrics() {
    setLoading(true);
    try {
      await Promise.all([fetchBudgetAlerts(), fetchActivePayroll(), fetchDailyHoursControl()]);
      const { data: activeProjects } = await supabase.from('projects').select('id').eq('is_archived', false);
      const activeProjectIds = (activeProjects || []).map((p) => p.id);
      if (activeProjectIds.length === 0) {
        setTeamMetrics([]);
  ...
  return (
    <div className="p-6">
      ...
      {isAdmin && dailyHoursControl.length > 0 && (
        <div className="mb-6 bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold mb-4 flex items-center">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Meta: {TARGET_HOURS_PER_DAY} horas/día. Verde oscuro = planificado antes. Verde claro = asignado hoy mismo.
          </p>
